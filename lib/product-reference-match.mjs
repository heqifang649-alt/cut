...
function productReferenceNameScore(file, keys) {
  ...
  for (const key of keys) {
    ...
    // Imported reference photos are often named like "样衣1" while the
    // deterministic product session is simply "1". Restrict suffix matching
    // to numeric product IDs so a short generic label cannot cross-match.
    if (/^\d+$/.test(key) && stem.endsWith(key)) best = Math.max(best, 55 + key.length);
    ...
    else if (/^(?:logo|标志|文字|text)(?:[-_\s\d].*)?$/i.test(suffix)) best = Math.max(best, 65 + key.length);
    else if (/^(?:图案|印花|细节|侧面|graphic|detail|details|side)(?:[-_\s\d].*)?$/i.test(suffix)) best = Math.max(best, 60 + key.length);
  }
  return best;
}
...
export function productReferenceType(file) {
  const name = evidenceStem(file?.relativePath || file?.name);
  if (/(?:正面?|front)/i.test(name)) return "front";
  if (/(?:反面?|背面|back)/i.test(name)) return "back";
  if (/(?:logo|标志|文字|text)/i.test(name)) return "logo_text";
  if (/(?:图案|印花|细节|detail|details)/i.test(name)) return "graphic_detail";
  return "garment_structure";
}
...
export function representativeProductReferencesForGroup(group, productReferenceFiles, limit = 5) {
  const max = Math.max(1, Number(limit) || 1);
  const matched = productReferencesForGroup(group, productReferenceFiles, Math.max(max * 3, max));
  const selected = [];
  const selectedTypes = new Set();
  for (const file of matched) {
    const type = productReferenceType(file);

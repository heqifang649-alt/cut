...
export class AiProviderAdapter {
  ...
  async complete({ model, prompt, images = [], jsonSchema, protocolMode = this.config.protocolMode, capability } = {}) {
    ...
    const runResponses = async () => {
      ...
      const response = await this.raw("/responses", body, { retryLimit: protocolMode === "auto" ? 0 : undefined });
    ...
    for (const chosenModel of candidates) {
      ...
      if (basic) {
        ...
        if (probeImages.length) {
          try {
            const singleImage = await this.complete({ model: chosenModel, prompt: singleImagePrompt, images: [probeImages[0]], protocolMode: this.config.protocolMode });
          ...
          if (capabilities.VISION_INPUT === "PASS") {
            try {
            const multiImage = await this.complete({ model: chosenModel, prompt: multiImagePrompt, images: probeImages.length > 1 ? probeImages : [probeImages[0], probeImages[0]], protocolMode: this.config.protocolMode });

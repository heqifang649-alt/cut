...
| TC-P1-VISION-PROTOCOL-BOUNDARY | Root PM / adapter reviewer | P1D | complete / local verified | Model-and-capability-scoped protocol cache, controlled Responses-to-Chat fallback, fallback telemetry, and 21 focused adapter tests. |
| TC-P1-VISION-CAPABILITY-DISCOVERY | Root PM / Eval | TC-P1-VISION-PROTOCOL-BOUNDARY | superseded evidence | Initial bounded run stopped after a shared circuit opened; it is retained as historical evidence only. The corrected discovery is recorded in `TC-P1-VISION-DISCOVERY-CORRECTION`. |
| TC-P1-VISION-DISCOVERY-CORRECTION | Root PM / Eval | TC-P1-VISION-PROTOCOL-BOUNDARY | complete | Discovery order forces `gpt-5.6-sol` first, isolates discovery retry/circuit budget, distinguishes dispatched/unattempted models, and returns null selection unless a model passes. Regression tests pass. |
...
| P1 Gate | Root PM / Eval | P1A, TC-P1-API-ROOT-CAUSE, P1D, TC-P1D-STRICT-SINGLE-MODEL, TC-P1-CIRCUIT-IDENTITY-AND-HALF-OPEN, TC-P1-VISION-PROTOCOL-BOUNDARY, TC-P1-VISION-CAPABILITY-DISCOVERY | blocked provider availability / capability boundary | Adapter fallback is locally verified. The bounded real discovery reached both text protocols but each returned 503; `VISION_INPUT`, `MULTI_IMAGE`, semantic output and reliability remain unverified. Streaming and usage metadata are non-blocking. |
| P1 Vision Gate | Root PM / Eval | TC-P1-VISION-DISCOVERY-CORRECTION | pass | `gpt-5.6-sol` was the first actual candidate and passed TEXT, real single-image, real multi-image, semantic-shot.v1 via validated JSON fallback, and bounded reliability. Pilot Primary is frozen; no further model discovery is required. |
| P2-SCHEDULER-SEMANTIC-EVIDENCE | Treatment B implementation | P1 Vision Gate | complete / local verified | Optional semantic evidence is consumed by the deterministic Scheduler: unusable shots are hard-rejected, low product match is guarded, and remaining candidates use conservative versionless signals. Legacy and Control A paths are unchanged. |
| P2 | Eval / benchmark | P2-SCHEDULER-SEMANTIC-EVIDENCE | next / real evidence required | Run independently labelled real semantic evaluation; keep 10/5/5/5 as a P2 quality gate and do not alter the frozen semantic contract. |
| P2-DATASET-INVENTORY | Eval / benchmark | P1 Vision Gate | complete / labels missing | Inventory found 31 real batches and 2061 media files (705 images, 674 videos plus derived files), but no independently labelled 10/5/5/5 manifest. No ground truth was fabricated. |
| P2-PROBE-EXECUTION | Provider / QA | P2-DATASET-INVENTORY | blocked | Run the same frozen prompt/schema on the real independently labelled P2 cohort only after dataset readiness; record semantic quality, latency, retries, errors, and JSON stability. |
| PM-REVIEW-P2 | Root PM / independent acceptance | P2-DATASET-INVENTORY, P2-PROBE-EXECUTION | pending | Approve P2 evidence only when input hashes, labels, observations, and metric calculations are independently reviewable. |
